...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      ...
      editorSubs.add(editor.onDidSave(() => this.calculateCost(editor, language)));
      ...
      this.calculateCost(editor, language);
    }));
  ...
  calculateCost(editor, language) {
    const editorData = this.editors.get(editor);
    const content = editor.getText();

    this.cleanupEditorImportCosts(editorData);

    editorData.importCosts = importCost(editor.getPath(), content, language);
  ...
  updatePositions(editor) {
    ...
    if (this.editors.has(editor) && this.editors.get(editor).packages) {
